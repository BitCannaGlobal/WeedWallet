    ...
    >
      <sequential-entrance>
        <v-row justify="space-around">
          <v-col class="text-h6 text-md-h5 text-lg-h4">Buddhead NFT Collection</v-col>
          <v-col>
      ...
      <v-row justify="space-around">
        <v-col>
          <v-card class="accent">
            ...
            <v-card-text class="text-right text-h5">
              {{ collectionData.data?.collection.floorPrice / 1000000 }} STARS
            </v-card-text>
          </v-card>
        </v-col>
        <v-col>
          <v-card class="accent">
            ...
            <v-card-text class="text-right text-h5">
              {{ collectionData.data?.collection.stats.bestOffer / 1000000 }} STARS
            </v-card-text>
          </v-card>
        </v-col>
        <v-col>
        ...
        <v-col>
          <v-card class="accent">
            ...
            <v-card-text class="text-right text-h5">
              {{ collectionData.data?.collection.numTokensForSale }} Buddhead
            </v-card-text>
          </v-card>
        </v-col>
      </v-row>
...
async beforeMount () {
    ...
    let finalData = []
    await axios.post('https://graphql.stargaze-apis.com/', {
      "query":"query ExampleQuery($address: String!) { collection(address: $address) { contractAddress stats { bestOffer numOwners } numTokensForSale numTokensAlive name floorPrice } }",
      "variables": {
        "address": "stars1w4dff5myjyzymk8tkpjrzj6gnv352hcdpt2dszweqnff927a9xmqc7e0gv"
      }
    })
