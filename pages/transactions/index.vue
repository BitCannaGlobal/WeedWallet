    ...
    <v-expansion-panels>
      ...
      <v-expansion-panel-header v-slot="{ open }">
        ...
        <v-expansion-panel-content>
            ...
            <template v-if="item.messageInfo.type === '/cosmos.bank.v1beta1.MsgSend'">
            ...
            <template v-if="item.messageInfo.type === '/cosmos.staking.v1beta1.MsgDelegate'">
              <v-simple-table class="accent">
                <template v-slot:default>
                  <thead>
                    <tr>
            ...
            <template v-if="item.messageInfo.type === '/cosmos.distribution.v1beta1.MsgWithdrawDelegatorReward'">
              <v-simple-table class="accent">
                <template v-slot:default>
                  <thead>
                    <tr>
...
const { toAscii, toHex, fromHex, fromUtf8, fromRfc3339 } = require("@cosmjs/encoding")
...
export default {
  ...
  async mounted () {
    ...
    if (this.logged) {
      ...
      const resultSender = await axios(cosmosConfig[0].apiURL + '/cosmos/tx/v1beta1/txs?events=message.sender=%27'+this.accounts[0].address+'%27&limit=20&order_by=2' )
      const resultRecipient = await axios(cosmosConfig[0].apiURL + '/cosmos/tx/v1beta1/txs?events=transfer.recipient=%27'+this.accounts[0].address+'%27&limit=20&order_by=2')
