  ...
  <div>
    ...
    >
      <template v-slot:activator="{ on, attrs }">
        ...
        Undelegate {{ selectedDelegation }} {{ config[0].coinLookup.viewDenom }}
        </v-btn>
      </template>
          ...
          <v-simple-table>
            <template v-slot:default>
              ...
              <tbody>
                <tr>
                  <td>Rewards</td>
                  <td>{{ selectedDelegation }} {{ config[0].coinLookup.viewDenom }}</td>
                </tr>
              </tbody>
            </template>
          </v-simple-table>
...
import {
  ...
  export default {
    props: ['chainIdProps', 'amountAvailable', 'coinIcon', 'selected', 'finalAmount'],
    data: (instance) => ({
      e1: 1,
      eError: true,
      dialog: false,
    ...
    selected(newSelected, oldSelected) {
      ...
      newSelected.forEach(async (item) => {
        let findDelegation = delegation.find(element => element.op_address === item)
        selectedDelegation += Number(findDelegation.share)
        finalUndelegate.push({ valAddress: findDelegation.op_address, amount: findDelegation.share })
      })
      ...
      validate () {
          ...
          (async () => {
            ...
            try {
              ...
              const result = await client.signAndBroadcast(myAccount, copieUndelegate, fee, '')
              this.e1 = 2
