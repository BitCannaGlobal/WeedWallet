    ...
    >
      <template v-slot:activator="{ on, attrs }">
        <v-btn
          ...
          v-on="on"
        >
          <v-icon class="mr-2">mdi-send-circle</v-icon> Vote for proposal {{ idProposal }}
        </v-btn>
      </template>
      <v-card>
        <v-card-title>
...
//   }
  export default {
    props: ['chainIdProps', 'amountAvailable', 'coinIcon', 'idProposal', 'cardsVote'],
    data: (instance) => ({
      selected: [],
      dialog: false,
      dislableSend: true,
      ...
      validate () {    
        ...
        if (this.$refs.form.validate() === true) {
          (async () => {
            ...
            try {          
              const result = await client.signAndBroadcast(accounts[0].address, voteSend, fee, '')       
